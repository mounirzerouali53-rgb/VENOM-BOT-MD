...
// دالة لطلب رقم الهاتف من المستخدم
...
function askForPhoneNumber() {
  ...
  return new Promise((resolve) => {
    ...
    rl.question('📱 الرجاء إدخال رقم هاتفك مع مفتاح الدولة (مثال: 212723302406): ', (phoneNumber) => {
...
async function startBot() {
  ...
  if (config.sessionID && config.sessionID.startsWith('KnightBot!')) {
    ...
    try {
      ...
      // Continue with normal pairing flow if session processing fails
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    // If we get a QR code but we want pairing code, we'll request pairing code

    if (qr && !fs.existsSync(sessionFile)) {

      console.log('📱 Generating pairing code instead of QR...');

      

      // ✅ استخدام الرقم من config.js مباشرة

      let phoneNumber = '';

      

      // جرب الحصول على الرقم من config

      let configNumber = config.ownerNumber || config.phoneNumber;

      

      // إذا كان configNumber مصفوفة، خذ أول عنصر

      if (Array.isArray(configNumber)) {

        configNumber = configNumber[0];
      ...
      if (configNumber) {
        ...
        // تحويل الرقم إلى نص وتنظيفه

        phoneNumber = String(configNumber).replace(/\D/g, '');

        console.log(`📱 Using phone number from config: ${phoneNumber}`);

      }

      

      // إذا ما كانش الرقم صالح، اطلبه من المستخدم

      if (!phoneNumber || phoneNumber.length < 10) {

        console.log('📱 Phone number not found in config, please enter it manually:');
      ...
      if (phoneNumber && phoneNumber.length >= 10) {
        ...
        try {
          ...
          console.log('║     🔐 رمز الاقتران الخاص بك     ║');
          ...
          console.log('📱 ثم أدخل الرمز المكون من 8 أرقام أعلاه\n');

        } catch (error) {

          console.error('❌ فشل في إنشاء رمز الاقتران:', error.message);

        }

      } else {

        console.error('❌ رقم هاتف غير صالح. تأكد من config.js أو أدخل رقم صحيح.');
    ...
    if (connection === 'close') {
      ...
      console.log(`🤖 Bot Name: ${config.botName || 'Bot'}`);

      console.log(`⚡ Prefix: ${config.prefix || '.'}`);

      

      const ownerNames = Array.isArray(config.ownerName) ? config.ownerName.join(', ') : config.ownerName || 'Owner';
      ...
      if (config.autoBio) {
        ...
        await sock.updateProfileStatus(`${config.botName || 'Bot'} | Active 24/7`).catch(() => {});
