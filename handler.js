...
const getMessageContent = (msg) => {
  ...
  if (m.viewOnceMessageV2Extension) m = m.viewOnceMessageV2Extension.message; // سطر جديد
  if (m.viewOnceMessage) m = m.viewOnceMessage.message;
  if (m.documentWithCaptionMessage) m = m.documentWithCaptionMessage.message;
  
  // دعم حاوية الأزرار التفاعلية (سطر جديد مهم)
  if (m.interactiveMessage) return m; 
...
const handleMessage = async (sock, msg) => {
  try {
   ...
   // Auto-Reply System (منشن البوت أو كلمة بوت)
// Auto-Reply System (منشن البوت أو كلمة بوت)
try {
    const autoReplyCmd = require('./commands/owner/autoreply');
    await autoReplyCmd.checkAutoReply(sock, msg);
} catch (e) {}

// فلتر الكلام الفاحش - سطرين فقط
try {
    const badWordsFilter = require('./commands/admin/badwords');
    await badWordsFilter.checkBadWords(sock, msg);
} catch (e) {}
      // منع الروابط
try {
    const antilink = require('./commands/admin/antilink');
    await antilink.checkAntilink(sock, msg);
} catch (e) {}
      // كتم العضو
try {
    const katm = require('./commands/admin/katm');
    await katm.checkMuted(sock, msg);
} catch (e) {}
   ...
   let body = '';
       // السماح بمرور الرسالة إذا كان هناك body (ناتج عن ضغط زر) حتى لو لم يكن هناك نص تقليدي
    if (!body && (!content || actualMessageTypes.length === 0)) return;
    
    // 🔹 Button response should also check unwrapped content
    // --- معالجة شاملة للنصوص والأزرار (VENOM SYSTEM) ---
    const btnResponse = 
    ...
    const nativeFlowResponse = 
      ...
      (content.interactiveMessage && content.interactiveMessage.nativeFlowMessage);

    if (btnResponse) {
      body = btnResponse.selectedButtonId || btnResponse.selectedId || btnResponse.singleSelectReply?.selectedRowId || '';
    } else if (nativeFlowResponse) {
      try {
    ...
    // إذا لم يكن هناك ضغط زر، نأخذ النص العادي
    if (!body) {
    ...
    // ====== معالج الأزرار التفاعلية (Interactive Buttons Handler) ======
    // يجب أن يكون قبل فحص الـ prefix لأن ضغطات الأزرار لا تبدأ بـ prefix
    ...
    if (interactiveResponse) {
      ...
      // ━━━ محاولة 1: nativeFlowResponseMessage (القائمة المنسدلة single_select) ━━━
      try {
        const nativeFlow = interactiveResponse.nativeFlowResponseMessage;
        if (nativeFlow?.paramsJson) {
          const params = JSON.parse(nativeFlow.paramsJson);
          buttonId = params.id || params.selectedId || '';
        }
        if (!buttonId && nativeFlow?.name) {
          buttonId = nativeFlow.name;
        }
      } catch (e) { /* تجاهل خطأ الـ parse */ }

      // ━━━ محاولة 2: buttonsResponseMessage (quick_reply الشكل البسيط) ━━━
      if (!buttonId) {
        try {
      ...
      // ━━━ محاولة 3: listResponseMessage (single_select بديل) ━━━
      if (!buttonId) {
        try {
      ...
      // ━━━ محاولة 4: templateButtonReplyMessage ━━━
      if (!buttonId) {
        try {
          const tplResp = msg.message?.templateButtonReplyMessage;
          if (tplResp) {
            buttonId = tplResp.selectedId || '';
          }
        } catch (e) {}
      }

      // ━━━ محاولة 5: selectedButtonId / selectedId مباشرة ━━━
      if (!buttonId) {
        buttonId = interactiveResponse.selectedButtonId || 
                   interactiveResponse.selectedId || '';
      }

      console.log('[DEBUG] interactiveResponse FULL:', JSON.stringify(interactiveResponse, null, 2));
      console.log('[DEBUG] msg.message keys:', Object.keys(msg.message || {}));
      console.log('[Interactive Button] buttonId المستخرج:', buttonId);

      if (buttonId) {
        // دالة مساعدة لتنفيذ الأوامر عبر الأزرار
        const executeCmd = async (cmdName, cmdArgs = []) => {
          const cmd = commands.get(cmdName);
          if (cmd) {
            await cmd.execute(sock, msg, cmdArgs, {
              from,
              sender,
              isGroup,
              groupMetadata,
        ...
        // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
        // معالجة أزرار المساعدة help_cat_N
        // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
        if (buttonId.startsWith('help_cat_')) {
          const catNumber = buttonId.replace('help_cat_', '');
          await executeCmd('مساعدة', [catNumber]);
          return;
        }

        // زر الرجوع للقائمة الرئيسية
        if (buttonId === 'help_main') {
          await executeCmd('مساعدة', []);
          return;
        }

        // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
        // معالجة أزرار منع الروابط antilink
        // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
        if (buttonId.startsWith('antilink_')) {
          const groupSettings = database.getGroupSettings(from);

          if (buttonId === 'antilink_enable') {
            if (groupSettings.antilink) {
              await sock.sendMessage(from, { text: '⚠️ *منع الروابط مفعل بالفعل*' }, { quoted: msg });
            } else {
          ...
          if (buttonId === 'antilink_disable') {
            if (!groupSettings.antilink) {
              await sock.sendMessage(from, { text: '⚠️ *منع الروابط معطل بالفعل*' }, { quoted: msg });
            } else {
              database.updateGroupSettings(from, { antilink: false });
              await sock.sendMessage(from, { text: '❌ *تم تعطيل منع الروابط*' }, { quoted: msg });
            }
            return;
          }

          if (buttonId === 'antilink_set_kick') {
            database.updateGroupSettings(from, { antilinkAction: 'kick', antilink: true });
            await sock.sendMessage(from, { text: `🚫 *تم التعيين: طرد فوري*
سيتم طرد أي عضو ينشر رابطاً ممنوعاً فوراً` }, { quoted: msg });
            return;
          }

          if (buttonId === 'antilink_set_delete_3') {
            database.updateGroupSettings(from, { antilinkAction: 'delete', maxWarnings: 3, antilink: true });
            await sock.sendMessage(from, { text: `🗑️ *تم التعيين: حذف + تحذير*
الحد الأقصى: 3 تحذيرات` }, { quoted: msg });
            return;
          }

          if (buttonId === 'antilink_set_delete_5') {
            database.updateGroupSettings(from, { antilinkAction: 'delete', maxWarnings: 5, antilink: true });
            await sock.sendMessage(from, { text: `🗑️ *تم التعيين: حذف + تحذير*
الحد الأقصى: 5 تحذيرات` }, { quoted: msg });
            return;
          }

          if (buttonId === 'antilink_status') {
            await executeCmd('منع_روابط', ['حالة']);
            return;
          }

          if (buttonId === 'antilink_list') {
            await executeCmd('منع_روابط', ['قائمة']);
            return;
          }
        ...
        switch (buttonId) {
          ...
          default:
            // إذا كان الـ ID لا يطابق أي حالة، ابحث عنه كأمر
            const btnCommand = commands.get(buttonId);
            if (btnCommand) {
              await executeCmd(buttonId);
            }
            return;
        }
      }
    }
    // ====== نهاية معالج الأزرار التفاعلية ======
...
// Antilink handler
// Antilink handler - محدث لاستخدام الكود الجديد
